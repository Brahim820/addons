...
#                             id = recored['id']
#                             del recored['id']
#                             values[field].append((1, id, recored)) if len(
#                                 recored) else values[field].append((2, id))
#                         else:
#                             values[field].append((0, 0, recored))
#     return values
...
def object_read(model_name, params, status_code):
    ...
    data = request.env[model_name].search_read(
...
def object_read_one(model_name, rec_id, params, status_code):
    ...
    data = request.env[model_name].search_read(
        domain=[('id', '=', rec_id)], fields=fields)
    if data:
...
# /api/auth/token        POST    - Login in odoo and get access tokens
...
class APIController(http.Controller):
    ...
    def token(self, **post):
        ...
        params = {key: post.get(key) for key in post if post.get(key)}
        db, username, password = post.get(
            'db'), post.get('login'), post.get('password')
        if not all([db, username, password]):
            # Empty 'db' or 'username' or 'password:
            return invalid_response(400, 'empty_db_or_username_or_password', 'Empty value of db or username or password!')
        ...
        # Generate tokens
        access_token = request.env['api.access_token']._get_access_token(
    ...
    def delete(self, **post):
        ...
        access_token = request.httprequest.headers.get('access_token')
        print('*************************************************', post, request.httprequest.headers)
        access_token_data = request.env['api.access_token'].sudo().search(
            [('token', '=', access_token)], order='id DESC', limit=1)
